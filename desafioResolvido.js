...
const paisAfricano = olympicsMedalTable.customFind(medalist => medalist.continent === "AFRICA").country;
console.log(paisAfricano);

// 2 - Crie um algoritmo que retorne o total de medalhas por país
const medalhasPorPais = olympicsMedalTable.customMap(country => country.gold + country.silver + country.bronze)
console.log(medalhasPorPais);

// 3 - Crie um algoritmo para encontrar os países que conquistaram mais que 10 medalhas de ouro
const paisesCom10MedalhasOuroNoMinimo =  olympicsMedalTable.customFilter(medals => medals.gold > 10)
...
const paisesCom30MedalhasNoMinimo = olympicsMedalTable.customFilter(country =>(country.gold + country.silver + country.bronze) >= 30)
                                                      .customMap(name => name.country)
...
/*Nessa questão tive dúvidas, pois o enuciado pede para verificar se o continente América do Sul conquistou pelo menos 20 medalhas de ouro, porém a constante se chama "paisesComPeloMenos20MedalhasDeOUro" o que me deu a impressão de que teria que verificar quantos países tem pelo menos 20 medalhas de ouro. Bem, na dúvida fiz os dois.*/

// algoritmo que verifica quantos países tem mais de 20 medalhas de ouro
const paisesComPeloMenos20MedalhasDeOUro = olympicsMedalTable.customFilter(id => id.gold >= 20)
                                                             .customMap(id => id.country)
console.log(paisesComPeloMenos20MedalhasDeOUro);

// algoritmo que verifica se o continente América do Sul conquistou pelo menos 20 medalhas de ouro
const americaDoSulTemPeloMenos20MedalhasDeOUro = (olympicsMedalTable.customFilter(id => id.continent === "AMERICA DO SUL").customMap(id=> id.gold)
     .customReduce((total, quantity) => total + quantity)) >= 20
console.log(americaDoSulTemPeloMenos20MedalhasDeOUro);

/*
Resolução por Anderson Moura
email: andersonmouradasilva2@gmail.com 
*/